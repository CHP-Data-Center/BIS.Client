...
function getInternalSourceRoute(source) {
  ...
  // 1. Gói mua sắm công / Đấu thầu
  if (source.kind === 'procurement' || idStr.startsWith('IB') || idStr.startsWith('PL')) {
    if (idStr.startsWith('PL')) {
      return `/news/khlcnt?q=${encodeURIComponent(idStr)}`;
    }
    return `/news/tbmt?q=${encodeURIComponent(idStr)}`;
  }

  // 2. Dự án ADB
  if (source.kind === 'adb' || source.kind === 'adb_notice') {
    return `/adb/project/${encodeURIComponent(idStr)}`;
  }

  // 3. Dự án World Bank
  if (source.kind === 'worldbank') {
    return `/worldbank/project/${encodeURIComponent(idStr)}`;
  }

  // 4. Tin tức / Bài viết chi tiết (mặc định)
